    ...
    p.prototype.upload = function(e, t) {
        ...
        u = function() {
            ...
            function(e, a) {
                ...
                i.ajax({
                    ...
                    error: function() {
                        n++,
                        o.msg("<font color='black'>请求上传接口出现异常</font>"),
                        m(e),
                        u()
                    }
                })
        ...
        c = function() {
            ...
            p.timer = setInterval(function() {
                ...
                try {
                    ...
                    o.msg("<font color='black'>获取上传后的响应信息出现异常</font>"),
                    clearInterval(p.timer),
                    m()
                }
        ...
        d = function(e, i) {
            if (o.elemFile.next("." + s).remove(), r.value = "", "object" != typeof i) try {
                ...
                return i = {},
                o.msg("<font color='black'>请对上传接口返回有效JSON</font>")
            }
        ...
        if (v = 0 === v.length ? r.value.match(/[^\/\\]+\..+/g) || [] || "": v, 0 !== v.length) {
            ...
            case "file":
                if (h && !RegExp("\\w\\.(" + h + ")$", "i").test(escape(v))){
                	...
                	return o.msg("<font color='black'>选择的文件中包含不支持的格式</font>");
                }
                r.value = "";
                break;
            case "video":
                if (!RegExp("\\w\\.(" + (h || "avi|mp4|wma|rmvb|rm|flash|3gp|flv") + ")$", "i").test(escape(v))) return o.msg("<font color='black'>选择的视频中包含不支持的格式</font>"),
                r.value = "";
                break;
            case "audio":
                if (!RegExp("\\w\\.(" + (h || "mp3|wav|mid") + ")$", "i").test(escape(v))) return o.msg("<font color='black'>选择的音频中包含不支持的格式</font>"),
                r.value = "";
                break;
            default:
                if (layui.each(v,
                ...
                layer.closeAll('loading');	
                return o.msg("<font color='black'>选择的图片中包含不支持的格式</font>");
                } 
                r.value = ""
            }
            if (o.fileLength = function() {
                ...
                i
            } (), l.number && o.fileLength > l.number) return o.msg("<font color='black'>同时最多只能上传的数量为：" + l.number+"</font>");
            if (l.size > 0 && !(a.ie && a.ie < 10)) {
                ...
                }), F) return o.msg("<font color='black'>文件不能超过" + F+"</font>")
            }
            y()
        }
    },
